...
export default function PostFoodPage() {
  ...
  const categories = [
    {
      id: 'red' as const,
      label: 'High Priority',
      description: 'Fast Spoilage',
      color: 'border-red-500 bg-red-50 hover:bg-red-100',
      selectedColor: 'border-red-600 bg-red-100 ring-4 ring-red-200',
    },
    {
      id: 'yellow' as const,
      label: 'Medium Priority',
      description: 'Moderate Shelf Life',
      color: 'border-yellow-500 bg-yellow-50 hover:bg-yellow-100',
      selectedColor: 'border-yellow-600 bg-yellow-100 ring-4 ring-yellow-200',
    },
    {
      id: 'green' as const,
      label: 'Low Priority',
      description: 'Packed Food',
      color: 'border-green-500 bg-green-50 hover:bg-green-100',
      selectedColor: 'border-green-600 bg-green-100 ring-4 ring-green-200',
    },
  ];
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (new Date(formData.safeUntil) <= new Date(formData.cookedTime)) {
    ...
    toast.success('Food posted successfully. Nearby NGOs can now accept it.');
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <Input
                  id="foodName"
                  placeholder="e.g., Fresh Biryani, Sandwiches"
              ...
              <div>
                ...
                <Input
                  id="quantity"
                  placeholder="e.g., 50 servings, 100 packets"
                  ...
                  {categories.map((category) => (
                    <button
                      ...
                      className={`p-4 border-2 rounded-lg text-left transition-all ${
                        selectedCategory === category.id ? category.selectedColor : category.color
                      }`}
                    >
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Use a clear pickup address so volunteers can find you.</p>
              </div>

              <div className="flex gap-4">
                <Button type="submit" className="flex-1 bg-green-600 hover:bg-green-700 text-white" size="lg">
                  Notify Nearby NGOs
                </Button>
                <Button type="button" variant="outline" onClick={() => navigate('/donor-dashboard')} size="lg">
                  Cancel
                </Button>
              </div>
            </form>
